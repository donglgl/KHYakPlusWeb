...
function showButtonById(buttonId) {
    ...
    newButton.id = 'catebutton';
    newButton.innerHTML = '<img src="./img/cataimg.png" width="60px">' + buttonId;
    newButton.style.display = 'flex'; // 버튼을 Flex 컨테이너로 설정
    newButton.style.flexDirection = 'column'; // 요소들을 세로로 배치
    newButton.style.alignItems = 'center'; // 요소들을 수직 가운데 정렬
...
var itemCategory,itemName,packagingUnit
...
function addkitList(button) {
	...
	console.log("아");
	var row = button.parentNode.parentNode;
	 itemCategory = row.cells[0].textContent.trim();
	 itemName = row.cells[1].textContent.trim();
    packagingUnit = row.cells[2].textContent.trim();
...
function select1() {
    ...
    // 모달에서 필요한 정보 추출
    var endDate = document.getElementById("myDate1").value;
    var count = document.getElementById("quantity1").value;

    console.log(itemCategory);
    console.log(itemName);
    console.log(packagingUnit);

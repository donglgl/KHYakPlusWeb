...
import com.admin.layout.service.ApiService;
import com.admin.layout.vo.ItemData;
import com.admin.layout.vo.info2item;
import com.admin.layout.vo.infoitem;
import com.admin.layout.vo.selectItem;
...
public class addrestcontroller {
	...
	public String enrollItem(@RequestBody ItemData itemData) {
		...
		Date endDate = itemData.getEndDate();
		...
		// Kit 엔티티 생성 및 데이터 설정
		kit kit = new kit();
		kit.setEndDate(formattedEndDate);
		...
		kit.setRemainDate(remainDate); // 현재 날짜를 kit 엔티티의 endDate에 설정

		// Kit 엔티티 저장
		kitrepository.save(kit);

		return "redirect:/anotherMypage";
	}
	...
	public String receiveButtonId(@RequestParam String buttonId, Model model) {
		...
		individualcategory categ = new individualcategory();
